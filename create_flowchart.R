...
## Create flowchart ##
######################
# Creates a file, ofi-flowchart.pdf. Reference 
#
...
df [label = 'Mortality conferance (n=@@8)']
ofi [label = 'OFI (n=@@9)']
...
") %>% DiagrammeRsvg::export_svg() %>% charToRaw() %>% rsvg::rsvg_pdf("ofi-flowchart.pdf")
  ...
  return("Pdf created, reference in a R chunk via rmarkdown: knitr::include_graphics(file-name with citations) ")
}
